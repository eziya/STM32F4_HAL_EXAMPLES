...
/* VARIABLES */
volatile uint8_t mp3buffer[VS1053_DATABUFFERLEN];
volatile bool feedBufferLock = false;
volatile bool DREQFlag = false;
...
{
	if(GPIO_Pin == MP3_DREQ_PIN) DREQFlag = true;
}

void VS1053_Select()
{
	HAL_GPIO_WritePin(MP3_CS_PORT, MP3_CS_PIN, GPIO_PIN_RESET);
	HAL_Delay(1);
}

void VS1053_Deselect()
{
	HAL_GPIO_WritePin(MP3_CS_PORT, MP3_CS_PIN, GPIO_PIN_SET);
	HAL_Delay(1);
}
...
{
	// cancel all playback
	VS1053_SCI_Write(VS1053_REG_MODE, VS1053_MODE_SM_LINE1 | VS1053_MODE_SM_SDINEW | VS1053_MODE_SM_CANCEL);
...
{
	return (!playingMusic && mp3file.obj.fs != 0);
}
...
unsigned long VS1053_Mp3ID3Jumper(FIL &mp3) {
